...
int main() {
	...
	int Vs; //it will be the dimensional spatial volume = Vs*a^3
	...
	*/
	...
	for (int ii = 0; ii < directories.size(); ++ii) {
		ifstream input_file;
		input_file.open(directories[ii] + "rho_tot_" + name_input_file);
		if (!input_file) {
		...
		double n_accum = 0;
		int n_conf = 0;
		vector <double> value_rhok;
		double mean_rho_T3_value;
				...
				if (conf_id != conf_tmp) {
					...
					if (value_rhok.empty()) {
						...
						for (int kk = 0; kk < value_rhok.size(); ++kk) {
							mean_rho_T3_value += kk * value_rhok[kk];
							mean_rho_T3.push_back();
						...
						mean_rho_T3_value /= (pow(afm[ii], 3) * Vs);//AGGIUSTA DIMENSIONI
					}
					value_rhok.clear();
