...
int main() {
	...
	int skipLines_f = 1, skipLines_file_list_therm = 1; //= number of lines to skip while reading input file;
	...
	while (getline(file_list, line)) {
		...
		string single_path;
		int n_therm_reff, n_therm_imff;
		if (iss >> single_path >> n_therm_reff >> n_therm_imff) {
	...
	for (int ii = 0; ii < paths.size(); ii++) {
		...
		if (pos != string::npos) {
			name_tmp = paths[ii].substr(0, pos); //I remove extension using substr
		}
		if (pos_slash != string::npos) {
			name_tmp = name_tmp.substr(pos_slash + 1); //I remove extension using substr
		}
		...
		process_autocorr_block(
			paths[ii],
			"results/autocorr_check_" + name_tmp + ".txt",
